...
public:
    ...
    void subProcess(int sampleFrames)
    {
        ...
        const int requestedStages = clampi(
            static_cast<int>(std::lround(finiteOrZero(*stagesPin))),
            kMinStages,
            kMaxStages);
        ...
        const float routingTarget = finiteOrZero(*routingPin) >= 0.5f ? 1.0f : 0.0f;
        routingSmoother_.setTarget(routingTarget);

        diffusionSmoother_.setTarget(clampf(
            finiteOrZero(*diffusionPin), -kMaxDiffusion, kMaxDiffusion));

        feedbackSmoother_.setTarget(clampf(
            finiteOrZero(*feedbackPin), -kMaxFeedback, kMaxFeedback));

        modDepthSmoother_.setTarget(clampf(
            finiteOrZero(*modDepthPin), 0.0f, kMaxDelayMs));

        for (int i = 0; i < kMaxStages; ++i)
        {
            const float ms = clampf(
                finiteOrZero(*delayPins[static_cast<std::size_t>(i)]),
                0.0f,
                kMaxDelayMs);
        ...
        for (int s = 0; s < sampleFrames; ++s)
        {
            ...
            const float modulation = clampf(finiteOrZero(*modIn++), -1.0f, 1.0f);
...
{
    auto registration =
        Register<AllPassDiffuser>::withId(L"Pandocrator Multi AllPass Diffuser");
}
