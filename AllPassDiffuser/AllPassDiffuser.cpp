...
public:
    ...
    void subProcess(int sampleFrames)
    {
        ...
        // Global size multiplies the base delay times (and their static random
        // offsets) while leaving modulation depth in absolute milliseconds.
        ...
        for (int s = 0; s < sampleFrames; ++s)
        {
            ...
            for (int i = 0; i < kMaxStages; ++i)
            {
                ...
                // Optional normalized 4x4 Hadamard mixing between the first and
                // second four-stage banks. At Amount=0 the v6 path is unchanged.
                // We BLEND rather than add so engaging the matrix does not create
                // an automatic +6 dB-style energy increase.
                if (i >= 4 && i <= 7 && hadamardAmount > 0.0f)
                {
                    if (!hadamardReady)
                    {
                        ...
                        // Orthonormal 4x4 Hadamard transform (normalization = 1/2).
...
private:
    void updateSampleRateIfNeeded()
    {
        ...
        hadamardEnableSmoother_.setTime(kParameterSmoothingMs, currentSampleRate_);
        hadamardAmountSmoother_.setTime(kParameterSmoothingMs, currentSampleRate_);
...
{
    auto registration =
        Register<AllPassDiffuser>::withId(L"Pandocrator Multi AllPass Diffuser v7");
}
