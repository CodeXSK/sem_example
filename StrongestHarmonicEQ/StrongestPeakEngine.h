...
public:
    ...
    bool processSample(float input, float& output)
    {
        ...
        const bool newAnalysis = pushAnalysisSample(input);

        output = biquad_.process(input);
        return newAnalysis;
    }
...
private:
    ...
    {
        if (!(detected > 0.0f) || !std::isfinite(detected))
            return;

        if (!(trackedHz_ > 0.0f) || !std::isfinite(trackedHz_))
        {
            trackedHz_ = detected;
            return;
        }
